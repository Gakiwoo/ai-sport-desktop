 ...
 * Handles JWT auth, token refresh, and typed request/response.
 */
...
export class ApiClient {
  ...
  // Workouts
  async syncWorkouts(workouts: WorkoutSession[]) {
    return this.request<WorkoutSyncResponse>('POST', '/workouts/sync', {
      workouts,
    });
  }
  ...
  async pullWorkouts(since?: string) {
    ...
    return this.request<WorkoutPullResponse>('GET', `/workouts/sync${query}`);
  }

  async getWorkoutStats() {
    return this.request<WorkoutStatsResponse>('GET', '/workouts/stats');
  }

  // Pilot
  async listSchools() {
    return this.request<School[]>('GET', '/pilot/schools');
  }

  async listClassrooms(schoolId?: string) {
    const query = schoolId ? `?schoolId=${encodeURIComponent(schoolId)}` : '';
    return this.request<Classroom[]>('GET', `/pilot/classrooms${query}`);
  }
  ...
  async upsertClassroom(data: {
    ...
    return this.request<Classroom>('POST', '/pilot/classrooms', data);
  }

  async listStudents(classId?: string) {
    const query = classId ? `?classId=${encodeURIComponent(classId)}` : '';
    return this.request<Student[]>('GET', `/pilot/students${query}`);
  }
  ...
  async upsertStudent(data: {
    ...
    return this.request<Student>('POST', '/pilot/students', data);
  }

  async batchImportStudents(data: {
    schoolId: string;
    classId: string;
    students: Array<{ name: string; studentNo?: string; gender?: string }>;
  }) {
    return this.request<{ imported: number; ids: string[] }>('POST', '/pilot/students/batch', data);
  }

  async listTasks(classId?: string) {
    const query = classId ? `?classId=${encodeURIComponent(classId)}` : '';
    return this.request<TrainingTask[]>('GET', `/pilot/tasks${query}`);
  }
  ...
  async upsertTask(data: {
    ...
    return this.request<TrainingTask>('POST', '/pilot/tasks', data);
  }

  async getTaskResults(taskId: string) {
    return this.request<TaskResultsResponse>('GET', `/pilot/tasks/${taskId}/results`);
  }
  ...
  async getClassSummary(classId: string, exerciseType?: string) {
    ...
    return this.request<ClassSummaryResponse>('GET', `/reports/class-summary?${params}`);
  }

  async getSchoolSummary(schoolId: string) {
    return this.request<SchoolSummaryResponse>(
      'GET',
      `/reports/school-summary?schoolId=${encodeURIComponent(schoolId)}`,
    );
  }
  ...
  async getStudentProgress(studentId: string, exerciseType?: string) {
    ...
    if (exerciseType) params.set('exerciseType', exerciseType);
    return this.request<StudentProgressResponse>('GET', `/reports/student-progress?${params}`);
  }
}
...
/** Singleton instance — configure with setTokens() after login */
